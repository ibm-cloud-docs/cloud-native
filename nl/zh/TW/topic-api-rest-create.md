...
不論是否在微服務架構中，雲端原生應用程式都會產生並使用 API。部分 API 視為內部或專用 API，部分則視為外部 API。
{:shortdesc}

內部 API 只用於防火牆環境內，讓後端服務彼此通訊。外部 API 呈現消費者的統一進入點，通常是由 {{site.data.keyword.apiconnect_long}} 這類工具所**管理**，可強制速率限制或其他使用限制。例如，[GitHub Developer API](https://developer.github.com/v3/){: new_window} ![外部鏈結圖示](../icons/launch-glyph.svg "外部鏈結圖示") 就是這種 API。它提供統一的 API，可一致地使用 HTTP 動詞、回覆碼及分頁行為，而不需要顯示內部實作詳細資料。此 API 可由一個大型應用程式支援，也可以由一組微服務支援。該詳細資料不會向消費者顯示，這使得 GitHub 可視需要自由發展其內部系統。

## RESTful API 的最佳作法
{: #bps-apis}

REST API 使用標準 HTTP 動詞來進行「建立」、「擷取」、「更新」及「刪除」(CRUD) 作業，且需要特別注意作業是否為「等冪」（可放心地重試多次）。

* POST 作業可用來建立或更新資源。POST 作業無法反覆呼叫。例如，如果使用 POST 要求來建立資源，且呼叫多次，則在每次呼叫後都會建立新的唯一資源。
* GET 作業必須能夠反覆呼叫，且不得產生負面影響。它們是要用來擷取資訊。不要使用含查詢參數的 GET 要求來變更或更新資訊。請改用 POST、PUT 或 PATCH 作業。
...
鑒於 API 是由軟體而非人類呼叫，因此以最有效且具效率的方式將資訊傳達給呼叫程式時請盡量小心。
...
請考量在回應中傳回哪些資料，以提高通訊效率。例如，使用 POST 要求建立資源時，回應必須在 Location 標頭中包括新建立資源的位置。建立的資源通常也會內含在回應內文中，因此不需要使用額外的 GET 要求來提取建立的資源。這同樣適用於 PUT 和 PATCH 要求。

### RESTful 資源 URI
{: #rest-uris}

關於 RESTful 資源 URI 的某些方面，存在不同的意見。一般而言，一致認為資源必須是名詞，而非動詞，且端點必須是複數。這為 CRUD 作業提供一個清楚的結構：
...
關係是使用階層式 URI 所建模，例如，`/accounts/16/credentials` 用於管理與帳戶相關聯的認證。

管理不符合一般結構的資源的作業沒有單一的方法。請進行最適合 API 消費者的作業。
...
* 接受不明屬性作為要求的一部分。
    > 如果服務以不必要的屬性呼叫您的 API，則會擲出這些值。在此情況下，傳回錯誤可能導致不必要的失敗，對使用者會造成負面影響。
* 傳回消費者所需的屬性
    > 避免公開內部服務詳細資料。會將消費者所需的屬性公開為 API 的一部分。
...
* 接受不明屬性作為回應的一部分。
    > 如果您收到非預期的變數，請不要發出異常狀況。如果回應包含您需要的資訊，則不需要在意伴隨的其他項目。

這些準則特別適用於 Java 這類高度類型化語言，其中，JSON 序列化和解除序列化通常會間接發生。例如，Jackson 庫或 JSON-P/JSON-B。請尋找語言機制，可讓您指定忽略不明屬性這類更寬鬆的行為，或是定義或過濾必須序列化的屬性。
...
如果遵循穩健性原則，則可能要很長的時間才能進行重大變更。進行該重大變更時，您可以選擇建置完全不同的服務，並隨著時間推移而淘汰原始服務。
