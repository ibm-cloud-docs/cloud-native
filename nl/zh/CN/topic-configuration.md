...
云本机应用程序必须可移植。您可以使用相同的固定工件来部署到多个环境，而无需更改代码或执行其他方面未测试的代码路径。
{:shortdesc}

[十二要素方法](https://12factor.net/){: new_window} ![外部链接图标](../icons/launch-glyph.svg "外部链接图标") 中的三个要素与此实践直接相关：

* 第一个要素建议正在运行的服务与已版本化的代码库之间建立一对一的关联。通过可原样部署到多个环境的已版本化代码库，创建固定部署工件，如 Docker 映像。
* 第三个要素建议将特定于应用程序的配置（是固定工件的一部分）与特定于环境的配置（在部署时提供给服务）相分离。
...
特定于应用程序的配置是固定工件的一部分。例如，在 WebSphere Liberty 上运行的应用程序定义了已安装功能部件的列表，这些功能部件用于控制在运行时中处于活动状态的二进制文件和服务。此配置特定于应用程序，并且包含在 Docker 映像中。Docker 映像还会定义侦听端口或已公开的端口，因为在启动容器时，执行环境会处理端口映射。 
...
服务配置和凭证（服务绑定）的管理因不同的平台而有所不同。Cloud Foundry 是将服务绑定详细信息存储在字符串化的 JSON 对象中，该对象会作为环境变量 `VCAP_SERVICES` 传递到应用程序。Kubernetes 是将服务绑定作为字符串化的 JSON 或者作为平面 `ConfigMap` 或 `Secret` 属性来存储，可以将其作为环境变量传递到容器化应用程序，或作为临时卷安装。对于本地开发（具有其自己的配置），本地测试通常是在云中运行的任何内容的简化版本。在不具备特定于环境的代码路径的情况下，以可移植方式应对这些变化因素可能很困难。
...
Helm 使用模板来创建 chart，以便可以稍后替换值。通过在 `mychart/templates/pod.yaml` 文件模板中使用以下示例，可以实现与前一个示例中相同的结果，但此示例对于在不同环境中使用具有更大灵活性：
...
现在，ConfigMap 是 Pod 中的一个独立的工件。它可以具有不同的生命周期。您可以更新或更改 ConfigMap 中的值，而不必因此重新部署 Pod。您还可以直接在命令行中更新和处理 ConfigMap，这在开发/测试/调试周期中会很有用。
...
Kubernetes 会为您执行 Base64 解码。在 Pod 中运行的容器在检索该环境变量时会识别 Base64 解码值。
