...
Mit Statusprüfungen kann innerhalb von automatisierten Systemen auf einfache Weise der Allgemeinzustand einer einzelnen Instanz untersucht werden. Das System reagiert auf die Statusprüfungsereignisse beispielsweise durch das Austauschen einer fehlgeschlagenen Instanz, das Aktualisieren der Routing-Tabellen oder das Kommunizieren des festgestellten Status an den Benutzer.
{:shortdesc}

In Kubernetes sind zwei integrierte Mechanismen für die Überprüfung des Status eines Containers definiert:

* Mit einen Readiness-Test (Bereitschaftstest) wird geprüft, ob der Prozess Anforderungen verarbeiten kann, die Routing-fähig sind. Kubernetes leitet keine Arbeit an einen Container weiter, bei dem der Readiness-Test fehlgeschlagen ist. Ein Readiness-Test schlägt fehl, wenn ein Service ausgelastet ist, die Initialisierung nicht beendet hat, überlastet ist oder keine Anforderungen verarbeiten kann. 
* Mit einem Liveness-Test (Aktivitätstest) wird geprüft, ob der Prozess erneut gestartet werden muss. Kubernetes stoppt einen Container mit fehlgeschlagenem Liveness-Test und startet ihn erneut, um sicherzustellen, dass die Pods in einem nicht funktionsfähigen Status beendet und ausgetauscht werden. Ein Liveness-Test schlägt fehl, wenn ein Service sich in einem nicht behebbaren Status befindet, wenn beispielsweise eine abnormale Speicherbedingung vorliegt. Mit einfachen Liveness-Tests, die immer die Antwort OK zurückgeben, können Container in einem inkonsistenten Status identifiziert werden. Dies kann vorkommen, wenn der Prozess, der Anforderungen bedient, abstürzt, während der Container noch aktiv ist.
...
Im Grunde basiert die Entwicklung von cloudnativen Anwendungen auf dem Prinzip, dass Containerprozesse tatsächlich fehlschlagen können, aber auf einfache Weise durch einen neuen Container ersetzt werden. Dies geschieht in Reaktion auf unerwartete Ereignisse wie Container- oder Maschinenfehler und aufgrund von Betriebsereignissen wie der horizontalen Skalierung und neuen Rollouts des Anwendungsimage. Readiness-Prüfungen stellen sicher, dass neue Containerinstanzen für Arbeit bereit sind, bevor Datenverkehr an diese Instanzen weitergeleitet wird. Mit denselben Prüfungen wird verhindert, dass der Datenverkehr an Instanzen weitergeleitet wird, die beendet wurden oder gelöscht werden.
...
Liveness-Prüfungen, die für die Identifizierung konzipiert sind, kommen seltener vor und stellen eher eine Ausnahme dar. Wenn ein Prozess in einen Status eintritt, bei dem keine Wiederherstellung möglich ist, ist der Prozess praktisch nicht funktionsfähig. Dies kann bei abnormalen Speicherbedingungen oder einem Deadlock aufgrund eines Programmierfehlers vorkommen. Die beste Wiederherstellung in diesem Fall ist die Beendigung des Containers. Dabei wird auch sämtliche Verarbeitung beendet, die zu dieser Zeit in dem Container ausgeführt wird. Das Beenden oder der Neustart von Schleifen kann in Anwendungen auftreten, bei denen Container nicht online gehen können, bevor sie beendet und ersetzt werden.
...
Eine aggressive Konfiguration einer Readiness-Prüfung, z. B. mit einer niedrigen Anfangsverzögerung, hat kaum Auswirkungen, da die zu frühe Ausführung des Tests nicht bewirkt, dass der Status der Readiness-Prüfung sich ändert. Eine aggressive Liveness-Prüfung, bei der zu früh ein Fehler gemeldet wird, bewirkt jedoch, dass der Status sich ändert und das System den Container früher beendet als beabsichtigt.
...
Statusprüfungen werden häufig ausgeführt. Dies kann zusätzlichen Systemaufwand verursachen. Mit Readiness- und Liveness-Tests wird nur die Funktionsfähigkeit von Unterstützungsservices wie Datenbanken oder anderen Mikroservices in ihrem Ergebnis geprüft, wenn kein akzeptabler Fallback vorhanden ist. Für einen Liveness-Test ist eine Prüfung der Unterstützungsservices eingeschlossen, wenn der lokale Container bei einem nicht erfolgreichen Ergebnis in einen nicht behebbaren Status eintreten würde. Bei einem Readiness-Test wird ein Unterstützungsservice nur geprüft, wenn der lokale Container Anforderungen nicht verarbeiten kann, wenn er fehlschlägt; die Bedingung jedoch behebbar ist.

Bei der Konfiguration der ursprünglichen Zeitverzögerung wird für einen Readiness-Test der niedrigste wahrscheinliche Wert und für eine Liveness-Prüfung der größte wahrscheinliche Zeitwert verwendet. Beispiel: Wenn ein Anwendungsserver normalerweise 30 Sekunden für den Start benötigt, beträgt eine typische Readiness-Verzögerung 10 Sekunden. Für die Liveness-Prüfung wird der Wert 60 Sekunden verwendet, um sicherzustellen, dass der Serverstart abgeschlossen ist, bevor auf Beendigung geprüft wird.
...
| *failureThreshold* | Wie oft Kubernetes versucht, einen Pod erneut zu starten, bevor aufgegeben wird, wenn der Pod startet und der Test fehlschlägt. Der Mindestwert ist 1 und der Standardwert ist 3. |
